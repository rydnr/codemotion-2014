...
(defun org-beamer--format-frame (headline contents info &optional background)
  "Format HEADLINE as a frame.
CONTENTS holds the contents of the headline.  INFO is a plist
used as a communication channel. BACKGROUND is the optional background"
  (let ((fragilep
...
as a communication channel."
  (unless (org-element-property :footnote-section-p headline)
    (let ((level (org-export-get-relative-level headline info))
	  ...
	  (background (org-element-property :BEAMER_BACKGROUND headline)))
      (cond
	...
	(format "\\note{%s}"
       ...
       ((= level frame-level)
	(org-beamer--format-frame headline contents info background))
